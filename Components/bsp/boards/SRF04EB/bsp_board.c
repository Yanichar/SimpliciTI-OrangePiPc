...
#if !defined(SW_TIMER)
{
  ...
  tickCount = (usec * BSP_CLOCK_MHZ);     /* Calculate the tick count. */
  ...
  /* Use union to set the byte registers without shifting. */
  qa.u16   = usec;
  T2CAPLPL = qa.u8[0];
  T2CAPHPH = qa.u8[1];
...
#if defined(MRFI_TIMER_ALWAYS_ACTIVE) && defined(SW_TIMER)
#error "MRFI_TIMER_ALWAYS_ACTIVE and SW_TIMER are mutually exclusive pre-processor ids, only one can be defined."
#endif
